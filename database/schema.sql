...
CREATE TABLE IF NOT EXISTS raf_patient_demographics (
  ...
  dual_type        VARCHAR(20)     NOT NULL DEFAULT 'non_dual' COMMENT 'non_dual | partial_dual | full_dual',
  disabled         TINYINT(1)      NOT NULL DEFAULT 0 COMMENT '1 = qualifies via disability',
  orec             VARCHAR(1)      NOT NULL DEFAULT '0' COMMENT 'Original Reason for Entitlement Code: 0=aged, 1=disabled, 2=ESRD, 3=disabled+ESRD',
  institutional    TINYINT(1)      NOT NULL DEFAULT 0 COMMENT '1 = SNF / nursing-facility resident',
  enrollment_source VARCHAR(30)    NOT NULL DEFAULT 'derived' COMMENT 'derived | raf_db | api_override | paste_mode_default',
...
-- Migration: add new enrollment columns to existing deployments.
-- Safe to run repeatedly (ALTER IGNORE or conditional logic handled at app level).
-- Run once against an already-deployed database:
--   ALTER TABLE raf_patient_demographics
--     ADD COLUMN IF NOT EXISTS dual_type       VARCHAR(20)  NOT NULL DEFAULT 'non_dual' AFTER dual_status,
--     ADD COLUMN IF NOT EXISTS orec            VARCHAR(1)   NOT NULL DEFAULT '0'        AFTER disabled,
--     ADD COLUMN IF NOT EXISTS institutional   TINYINT(1)   NOT NULL DEFAULT 0          AFTER orec,
--     ADD COLUMN IF NOT EXISTS enrollment_source VARCHAR(30) NOT NULL DEFAULT 'derived'  AFTER institutional,
--     ADD KEY IF NOT EXISTS idx_orec (orec);
