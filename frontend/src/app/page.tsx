...
export default function DashboardPage() {
  ...
  const { data: emrStatus, isLoading: emrStatusL } = useQuery({
    queryKey: ["emr-status"],
  ...
  const { data: rev, isLoading: revL, error: revErr } = useQuery({
    queryKey: ["revenue-opportunity", new Date().getFullYear()],
  ...
  const { data: providersData } = useQuery({
    ...
    queryFn: () => getProviders({ limit: 6 }),
    retry: 1,
    staleTime: 60_000,
  });

  const { data: suspectsData } = useQuery({
    queryKey: ["suspects-summary-open"],
    queryFn: () => getSuspectsSummary("open"),
    retry: 1,
    staleTime: 60_000,
  });
  ...
  const avgRaf = rev?.average_raf_score ?? (stats as unknown as Record<string, number>)?.avg_raf_score ?? 0;
  ...
  const rafTrend = useMemo(() => {
    if (!trends?.average_raf?.change_pct) return undefined;
    return { value: trends.average_raf.change_pct, label: "vs prior 30d" };
  }, [trends]);

  // Provider data
  const providers = useMemo(() => {
    if (providersData?.providers?.length) {
      return providersData.providers.slice(0, 4).map((p: { last_name: string; specialty?: string; patient_count?: number; avg_raf?: number | null; coding_rate?: number | null }) => ({
        name: `Dr. ${p.last_name}`,
        specialty: (p.specialty || "Internal Medicine") as string,
        patients: (p.patient_count ?? 0) as number,
        avgRaf: p.avg_raf != null ? Number(p.avg_raf).toFixed(2) : "\u2014",
        codingRate: p.coding_rate != null ? Math.round(p.coding_rate) : "\u2014",
      }));
    }
    return [
      { name: "Dr. Mitchell", specialty: "Internal Medicine", patients: 142, avgRaf: "1.24", codingRate: 94 },
      { name: "Dr. Patel", specialty: "Cardiology", patients: 98, avgRaf: "1.67", codingRate: 88 },
      { name: "Dr. Thompson", specialty: "Endocrinology", patients: 76, avgRaf: "1.45", codingRate: 82 },
      { name: "Dr. Garcia", specialty: "Pulmonology", patients: 64, avgRaf: "1.31", codingRate: 79 },
    ];
  }, [providersData]);
