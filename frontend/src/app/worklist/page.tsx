    ...
    <div style={{ padding: "20px 16px", maxWidth: 1280, margin: "0 auto" }} className="rci-page-pad-desktop">
      ...
      {data.items.length === 0 ? (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: 16 }}>
          {data.items.map((item) => <PatientCard key={item.patient_id} item={item} />)}
        </div>
      )}
    </div>
  );
...
function PatientCard({ item }: { item: WorklistItem }) {
  ...
  return (
    <Link href={`/patients/${item.patient_id}`} style={{ display: "block", padding: 16, borderRadius: 10, background: tokens.white, border: `1px solid ${tokens.slate200}`, textDecoration: "none", color: "inherit", transition: "box-shadow 120ms ease" }} className="hover-lift">
