/**
 * iter2-08 — admin dashboard verification.
 *
 * Confirms iter1 deduction fix: H1 "Population Health Intelligence" no longer
 * collides with hamburger button on mobile (414px viewport).
 *  - mobile scrollWidth = viewport (no horizontal overflow)
 *  - H1 x position >= 60 (clear of hamburger right edge)
 */
...
const SHOT_DIR = path.resolve(__dirname, "../../demo-shots/iter2-08-dashboard");

function ensureDir(d: string) { fs.mkdirSync(d, { recursive: true }); }

test.describe.configure({ mode: "serial" });

test("iter2 verify dashboard — desktop + mobile 414px", async ({ browser }) => {
  ensureDir(SHOT_DIR);
  ...
  const storageStatePath = "/tmp/iter2-dashboard-state.json";
  ...
  // H1 bounding box and visibility
