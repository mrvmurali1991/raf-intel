...
def dashboard_kpi_trends(
    ...
    try:
        with raf_cursor() as cur:
            ...
            for row in cur.fetchall():
                idx = _week_index(_date.fromisoformat(str(row["week_start"])[:10]))
                if 0 <= idx < weeks:
    ...
    def _wow_delta(series: list[float]) -> float | None:
        """% change from second-to-last to last bucket."""
