...
def normalize_code(code: str) -> str | None:
    ...
    Attempts several forms:
      1. The code as given (already may have a dot).
      2. Without any dot.
      3. With a dot inserted after character 3.

    Behaviour:
      * If the input passes through unchanged, it is returned as-is.
      * If normalisation changes the code, a WARNING is logged showing both
        the input and the chosen output.
      * If MORE THAN ONE variant is a valid ICD-10-CM item, a ``ValueError``
        is raised listing all candidates so the caller must disambiguate.
      * Codes shorter than 3 chars, longer than 8 chars, or containing any
        character outside ``[A-Z0-9.]`` are rejected: ``None`` is returned
        and the rejection is logged.
      * If none of the variants are valid items, the uppercased original is
        returned (preserving legacy behaviour for downstream callers that
        then re-check via ``is_valid_item``).
    """
    if not code:
        return code
    upper = _norm(code)

    # Reject clearly invalid shapes.
    if len(upper) < 3 or len(upper) > 8:
        logger.warning(
            "normalize_code rejected %r: length %d out of range [3,8]",
            code, len(upper),
        )
        return None
    if any(ch not in _VALID_CODE_CHARS for ch in upper):
        logger.warning(
            "normalize_code rejected %r: contains characters outside [A-Z0-9.]",
            code,
        )
        return None

    # Build candidate list (ordered, deduplicated).
    candidates: list[str] = []
    def _add(c: str) -> None:
    ...
    if len(upper) > 3 and "." not in upper:
        _add(upper[:3] + "." + upper[3:])

    # Find all variants that are valid ICD-10-CM items.
    ...
    if len(valid_variants) > 1:
        # Dotted vs dotless forms of the same code are equivalent notations,
        # not true ambiguity. Collapse them: if every dotless variant matches
        # a dotted variant after stripping dots, prefer the dotted form
        # (canonical CMS notation). Only raise when the underlying codes
        # actually differ.
        canonical = {v.replace(".", "") for v in valid_variants}
        if len(canonical) == 1:
            dotted = next((v for v in valid_variants if "." in v), valid_variants[0])
            return dotted
        raise ValueError(
            f"Ambiguous ICD-10-CM code {code!r}: multiple valid normalisations "
            f"found {valid_variants}. Caller must disambiguate."
        )
    ...
    if len(valid_variants) == 1:
        ...
        if chosen != upper:
            logger.warning(
                "normalize_code rewrote %r -> %r (input form was not a valid "
                "ICD-10-CM item as-given)",
                upper, chosen,
            )
        return chosen

    # No variant is a valid item — preserve legacy behaviour.
    return upper
...
# ---------------------------------------------------------------------------
# Minimal self-test for normalize_code
# ---------------------------------------------------------------------------

if __name__ == "__main__":
    logging.basicConfig(level=logging.WARNING)

    # 1. Clean dotted code should pass through unchanged when valid.
    clean = "E11.65"
    try:
        result = normalize_code(clean)
        assert result in (clean, clean.replace(".", "")), f"clean pass-through failed: {result!r}"
        print(f"OK  clean pass-through: {clean!r} -> {result!r}")
    except ValueError as exc:
        print(f"OK  clean code was ambiguous (acceptable): {exc}")

    # 2. Undotted form should normalise to dotted (or raise ambiguous).
    undotted = "E1165"
    try:
        result = normalize_code(undotted)
        print(f"OK  undotted normalised: {undotted!r} -> {result!r}")
        assert result is not None
    except ValueError as exc:
        print(f"OK  undotted was ambiguous (expected for bug demo): {exc}")

    # 3. Invalid characters should be rejected -> None.
    bad = "E11$65"
    result = normalize_code(bad)
    assert result is None, f"expected None for invalid chars, got {result!r}"
    print(f"OK  invalid chars rejected: {bad!r} -> None")

    # 4. Too-short code should be rejected.
    short = "E1"
    result = normalize_code(short)
    assert result is None, f"expected None for too-short, got {result!r}"
    print(f"OK  too-short rejected: {short!r} -> None")

    # 5. Too-long code should be rejected.
    long_code = "E11.65ABCD"
    result = normalize_code(long_code)
    assert result is None, f"expected None for too-long, got {result!r}"
    print(f"OK  too-long rejected: {long_code!r} -> None")

    # 6. Lowercase input should be upper-cased transparently.
    lower = "e11.65"
    try:
        result = normalize_code(lower)
        print(f"OK  lowercase normalised: {lower!r} -> {result!r}")
    except ValueError as exc:
        print(f"OK  lowercase was ambiguous: {exc}")

    print("normalize_code self-tests complete.")
