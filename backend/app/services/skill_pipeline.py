...
def _handle_lookup_hcc(icd10_code: str) -> dict:
    ...
    raf_weight = coefficients_default.get((f"HCC{cc}", MODEL)) or 0.0
...
def _handle_calculate_raf(
    ...
    try:
        ...
        demo_score = sum(
            ...
            and k not in result.interactions
        )
        ...
        # Interaction score: sum of all interaction term values.
        ...
        interactions_dict = {k: v for k, v in result.interactions.items()}
        interaction_score = sum(interactions_dict.values())
        ...
        interaction_details = []
        for term_name, indicator in (result.interactions or {}).items():
