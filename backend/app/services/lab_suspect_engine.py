...
def _threshold_met(value: float, rule: dict[str, Any]) -> bool:
    ...
    # Optional exclusive lower bound — value must be >= max_excl
...
def run_lab_suspect_scan(pid: int) -> dict[str, Any]:
    """
    ...
    logger.info(
        "run_lab_suspect_scan pid=%s: notes=%d vitals_rows=%d "
        "note_suspects=%d vitals_suspects=%d merged=%d",
        pid, notes_scanned, vitals_rows_checked,
