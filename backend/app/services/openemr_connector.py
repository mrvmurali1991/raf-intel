...
def search_patients(query: str, limit: int = 100, offset: int = 0) -> tuple[list[dict[str, Any]], int]:
    ...
    if is_pid:
        ...
        search_sql = """
            SELECT pid, fname, lname, mname, DOB, sex, race, ethnicity, ss,
...
def get_patient(pid: int) -> dict[str, Any] | None:
    ...
    sql = """
        SELECT
            pid, fname, lname, mname, DOB, sex, race, ethnicity,
            ss, street, city, state, postal_code,
            phone_home, phone_cell, email, providerID, date AS created_date
        FROM patient_data
        WHERE pid = %s
    """
    with openemr_cursor() as cur:
...
def get_patient_count() -> int:
    with openemr_cursor() as cur:
        cur.execute("SELECT COUNT(*) AS cnt FROM patient_data WHERE pid > 0")
        row = cur.fetchone()
    ...
    sql = """
        SELECT
            ...
            (SELECT COUNT(*) FROM form_clinical_notes fcn
             WHERE CAST(fcn.encounter AS UNSIGNED) = fe.encounter
...
CPT_CONDITION_HINTS: dict[str, dict] = {
    ...
    "92227": {"condition": "Diabetic retinal screening",       "icd10": "E11.3x", "hcc": "HCC37"},
}
...
def get_cpt_codes(pid: int) -> list[dict[str, Any]]:
    ...
    sql = """
        SELECT DISTINCT code, code_text, date, encounter, modifier, units, fee
        FROM billing
...
def get_all_billing(pid: int) -> list[dict[str, Any]]:
    ...
    sql = """
        SELECT code, code_type, code_text, date, encounter, modifier
        FROM billing
...
def get_medications(pid: int) -> list[dict[str, Any]]:
    ...
    for row in result:
        row["diagnosis_codes"] = _parse_diagnosis_field(row.get("diagnosis"))
    return result
...
def get_vitals_history(pid: int) -> list[dict[str, Any]]:
    """
...
def get_vitals_trends(pid: int) -> dict[str, Any]:
    """
...
def get_clinical_notes(encounter_id: int) -> list[dict[str, Any]]:
    ...
    # Generic clinical notes form (description column, encounter is varchar)
    sql_cn = """
        SELECT
            fcn.id,
            fcn.pid,
        ...
        WHERE CAST(fcn.encounter AS UNSIGNED) = %s AND (fcn.activity = 1 OR fcn.activity IS NULL)
    """
...
def get_labs(pid: int) -> list[dict[str, Any]]:
...
def get_patient_enrollment_info(pid: int) -> dict[str, Any]:
    ...
    try:
        ...
        for row in ins_rows:
            ins_type = str(row.get("type") or "").strip()
            plan     = str(row.get("plan_name")    or "")
            grp      = str(row.get("group_name")   or "")
            company  = str(row.get("company_name") or "")
            cms_id   = str(row.get("cms_id")       or "")
            combined = f"{plan} {grp} {company} {cms_id}"

            is_medicare = _insurance_contains(combined, _MEDICARE_KEYWORDS)
            is_medicaid = _insurance_contains(combined, _MEDICAID_KEYWORDS)

            # Prefer company name, fall back to plan name, then group name.
            display = (company.strip() or plan.strip() or grp.strip()) or None
    ...
    try:
        ...
        """
        with openemr_cursor() as cur:
            cur.execute(pos_sql, (pid,))
            pos_rows = cur.fetchall()

        pos_codes = [
            str(r.get("pos_code") or "").strip()
            for r in pos_rows
            if (r.get("pos_code") or "").strip()
        ]

        if any(p in _INSTITUTIONAL_POS for p in pos_codes):
            is_institutional = True
            logger.debug(
                "get_patient_enrollment_info: pid=%s institutional=True via POS %s",
                pid,
                [p for p in pos_codes if p in _INSTITUTIONAL_POS],
            )
    except Exception as exc:
        logger.debug(
            "get_patient_enrollment_info: pos_code check failed for pid=%s (%s)", pid, exc
        )
...
def get_problem_list(pid: int) -> list[dict[str, Any]]:
    """
...
def get_medication_diagnoses(pid: int) -> list[dict[str, Any]]:
    ...
    """
    sql = """
        SELECT DISTINCT p.diagnosis, p.drug, p.active
...
def get_all_patient_diagnoses(pid: int) -> list[dict[str, Any]]:
    ...
    try:
        ...
        logger.warning("get_all_patient_diagnoses error pid=%s: %s", pid, exc)
...
def get_hedis_compliance(pid: int, year: int) -> dict[str, Any]:
    ...
    # ------------------------------------------------------------------
    # Influenza -- annual; CDC recommends everyone 6 months+.
    # Stars/HEDIS focuses on 65+, but we flag due=True for all ages so
    # the UI can decide what to surface per its own eligibility rules.
    ...
    # ------------------------------------------------------------------
    # Pneumococcal -- lifetime series; due at age >= 65.
    # ------------------------------------------------------------------
    pneumo_records = [
        i for i in immunizations
    ...
    # ------------------------------------------------------------------
    # Zoster (Shingles) -- series of 2 RZV doses or 1 live-virus dose;
    # recommended for age >= 50.
    # Full compliance: 2x CVX 187/188 (Shingrix series) or 1x CVX 121
    # (Zostavax, no longer manufactured but historically administered).
    # ------------------------------------------------------------------
    zoster_records = [
        i for i in immunizations
...
def get_medication_diagnosis_gaps(pid: int, year: int) -> list[dict[str, Any]]:
    ...
    """
    rx_sql = """
        SELECT p.diagnosis, p.drug, p.active
        FROM prescriptions p
        WHERE p.patient_id = %s
          AND p.active = 1
          AND p.diagnosis IS NOT NULL
...
def get_sdoh_data(pid: int) -> dict[str, Any]:
    ...
    sql_zcodes = """
        SELECT DISTINCT code, code_text, MAX(date) AS last_billed
        FROM billing
        WHERE pid = %s
          AND activity = 1
