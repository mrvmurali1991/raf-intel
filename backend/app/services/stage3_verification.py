...
def _is_subsumed(dropped_code: str, existing_codes: set[str]) -> tuple[bool, str]:
    ...
    for existing in existing_codes:
        ...
        try:
            ...
            logger.debug("get_ancestors(%s) failed: %s", existing_norm, exc)
...
def _enrich_restored_code(code: str) -> dict:
    ...
    if hcc_set:
        ...
        coeff = coefficients_default.get((f"cna_hcc{hcc_num}", "CMS-HCC Model V28")) or coefficients_default.get((f"HCC{hcc_num}", "CMS-HCC Model V28")) or 0.0
        return {
            "hcc": f"HCC{hcc_num}",
            "hcc_label": label,
...
def _extract_basic_meat(icd10_code: str, description: str, note_text: str) -> dict:
    """Extract basic MEAT evidence from the clinical note for a restored code.

    Searches the note for sentences mentioning the condition and assigns them
    to the appropriate MEAT category based on keyword matching.
    """
    if not note_text:
        return {"meat": {"M": "", "E": "", "A": "", "T": ""}, "meat_completeness": 0, "meat_status": "missing"}

    note_lower = note_text.lower()
    desc_lower = (description or "").lower()

    # Build search terms from the ICD code and description
    search_terms = [icd10_code.lower()]
    # Add key phrases from description (e.g., "diabetes", "hyperglycemia", "kidney", "vascular")
    for word in desc_lower.split():
        if len(word) > 4 and word not in ("with", "type", "without", "other", "unspecified", "mellitus"):
            search_terms.append(word)
    # Add condition-specific synonyms
    _synonyms = {
        "diabetes": ["dm2", "dm", "diabetic", "insulin", "a1c", "hba1c", "glucose", "glargine", "metformin"],
        "heart": ["chf", "cardiac", "systolic", "diastolic", "bnp", "ejection", "nyha", "carvedilol", "furosemide"],
        "kidney": ["ckd", "renal", "egfr", "nephro", "creatinine", "dialysis"],
        "vascular": ["pvd", "claudication", "peripheral", "abi"],
        "hypertension": ["htn", "bp ", "blood pressure", "lisinopril", "antihypertensive"],
    }
    for key, syns in _synonyms.items():
        if key in desc_lower or any(t in desc_lower for t in syns):
            search_terms.extend(syns)

    # Find relevant sentences — search ALL sentences in the note
    ...
    # Categorize sentences into MEAT
    monitor_kw = {"bp ", "bps", "bnp", "hba1c", "a1c", "egfr", "labs", "monitor", "weight", "o2", "sat", "pulse", "vital", "measure", "level", "value", "result"}
    evaluate_kw = {"stage", "nyha", "class", "grade", "severity", "chronic", "acute", "history", "presents", "stable", "controlled", "improving", "worsening", "well controlled"}
    assess_kw = {"assess", "recapture", "confirm", "diagnos", "review", "evaluat", "baseline", "annual", "awv"}
    treat_kw = {"continu", "prescri", "medicat", "insulin", "dose", "mg", "units", "diet", "refer", "schedul", "follow", "glargine", "metformin", "furosemid", "carvedilol", "lisinopril", "current med", "plan:"}

    m_parts, e_parts, a_parts, t_parts = [], [], [], []
    for s in relevant:
        s_lower = s.lower()
        if any(kw in s_lower for kw in monitor_kw):
            m_parts.append(s)
        if any(kw in s_lower for kw in evaluate_kw):
            e_parts.append(s)
        if any(kw in s_lower for kw in assess_kw):
            a_parts.append(s)
        if any(kw in s_lower for kw in treat_kw):
            t_parts.append(s)

    # Fallbacks: if a category is empty, try broader assignment
    if not e_parts and relevant:
        e_parts = relevant[:1]  # Any mention of the condition = evaluation
    if not a_parts and relevant:
        # The fact that the code was in the note at all = assessment
        a_parts = [f"Documented in clinical note: {icd10_code} ({description})"]
    if not t_parts:
        # Search entire note for treatment keywords
        all_treat_kw = {"continu", "prescri", "medicat", "plan", "mg", "units", "dose"}
        for s in sentences:
            if any(kw in s.lower() for kw in all_treat_kw):
                t_parts.append(s)
                break

    meat = {
        "M": ". ".join(m_parts[:2]) if m_parts else "",
        "E": ". ".join(e_parts[:2]) if e_parts else "",
        "A": ". ".join(a_parts[:2]) if a_parts else "",
        "T": ". ".join(t_parts[:2]) if t_parts else "",
    }
    completeness = sum(1 for v in meat.values() if v)
    status = "complete" if completeness == 4 else "partial" if completeness >= 2 else "missing"

    return {"meat": meat, "meat_completeness": completeness, "meat_status": status}
...
def compute_quality_score(
    ...
    # Mild penalty for restored codes — the safety net caught them, which is good.
    # Only penalize slightly since the code WAS captured (by Stage 1 + Stage 3).
...
def run_stage3(
    ...
    for dropped in restored_codes:
        ...
        verified_by_code[code] = {
            ...
            # Attempt basic MEAT extraction from note for restored codes.
            **_extract_basic_meat(code, dropped.get("description") or _description(code), note_text),
            "stage1_found": True,
            "stage2_found": False,
            "stage3_restored": True,
