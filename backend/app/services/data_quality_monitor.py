...
The top-level ``run_all_checks(db, tenant_id)`` function executes every
...
def check_patients_without_icd(db: Session, tenant_id: int) -> dict[str, Any]:
            ...
            WHERE p.tenant_id = :tenant_id
              ...
              AND NOT EXISTS (
                    ...
                    WHERE d.patient_id = p.id
                      AND d.diagnosis_date >= (CURRENT_DATE - INTERVAL '365 days')
              )
            ORDER BY p.id
            LIMIT 1000
            """
        )
        rows = [r[0] for r in db.execute(sql, {"tenant_id": tenant_id}).fetchall()]
        return {
            "check": check,
            "severity": "warn",
            "count": len(rows),
...
def check_hccs_missing_meat(db: Session, tenant_id: int) -> dict[str, Any]:
...
def check_raf_score_outliers(db: Session, tenant_id: int) -> dict[str, Any]:
...
def check_future_dob(db: Session, tenant_id: int) -> dict[str, Any]:
...
def check_duplicate_patients(db: Session, tenant_id: int) -> dict[str, Any]:
...
def check_stale_crosswalk(db: Session) -> dict[str, Any]:
    """HCC/ICD-10 crosswalk effective_year is more than 1 year behind today."""
    check = "stale_crosswalk"
    try:
        sql = text("SELECT MAX(effective_year) FROM hcc_icd10_crosswalk")
        max_year = db.execute(sql).scalar()
...
def check_orphaned_hccs(db: Session, tenant_id: int) -> dict[str, Any]:
    ...
    try:
        sql = text(
            ...
            WHERE p.id IS NULL
              AND (h.tenant_id = :tenant_id OR :tenant_id IS NULL)
            ORDER BY h.id
            LIMIT 1000
            """
        )
        try:
            ...
            rows = [r[0] for r in db.execute(fallback).fetchall()]
        return {
            "check": check,
            "severity": "error",
            "count": len(rows),
...
_TENANT_CHECKS: list[Callable[[Session, int], dict[str, Any]]] = [
...
_GLOBAL_CHECKS: list[Callable[[Session], dict[str, Any]]] = [
    check_stale_crosswalk,
]


def run_all_checks(db: Session, tenant_id: int) -> list[dict[str, Any]]:
