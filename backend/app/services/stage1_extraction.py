...
Rule-based limitations (IMPORTANT)
----------------------------------
ICD-10 extraction here is purely lexical: regex + a simple preceding-
context window for negation/uncertainty cues.  This approach has known
failure modes:

  * Cannot resolve cross-sentence scope (e.g. "Patient was evaluated for
    chest pain. CHF was not found.") — the negation sits in a different
    sentence than the code mention.
  * Cannot distinguish family-history vs patient-history mentions unless
    the trigger phrase happens to fall inside the 60-char lookback.
  * Cannot understand temporal resolution ("DM2 in 2015, now well
    controlled off meds") beyond crude keywords like "resolved".
  * Double-negation ("no evidence of no CHF") will be mis-classified.
  * Abbreviations ("h/o" vs "history of") and punctuation variance can
    cause triggers to be missed.

These gaps produce both false positives (uncaught negations) and false
negatives (valid codes buried near the word "history").  The correct
long-term fix is a clinical NLP model trained for assertion / negation
detection (e.g. NegEx++ with a transformer backbone, medspaCy's
ConText algorithm, or a fine-tuned BioClinicalBERT assertion classifier)
integrated in Stage 2.  Until then, treat `negated_codes` /
`uncertain_codes` as review hints, not ground truth.
...
#
# Word boundaries (\b) correctly handle codes inside parentheses or square
# brackets, e.g. "(E11.9)" or "[E11.9]", because '(' ']' and similar are
# non-word characters so \b matches at the E/9 edges.  We still explicitly
# document this so future refactors don't break bracket-enclosed mentions.
_ICD10_PATTERN = re.compile(
    r"(?<![A-Za-z0-9])([A-TV-Z][0-9]{2}(?:\.[A-Z0-9]{1,4})?)(?![A-Za-z0-9])",
    re.IGNORECASE,
)

# ---------------------------------------------------------------------------
# Assertion context triggers (rule-based negation / uncertainty detection)
# ---------------------------------------------------------------------------
# These are matched case-insensitively against a ~60-char window BEFORE each
# ICD mention within the same sentence.  See module docstring for the known
# limitations of this approach and the recommended ML upgrade path.
...
# How many chars to look back from the ICD match for an assertion trigger.
_ASSERTION_LOOKBACK_CHARS = 60


def _preceding_window(text: str, match_start: int) -> str:
    """
    Return up to _ASSERTION_LOOKBACK_CHARS of text immediately preceding
    *match_start*, bounded by the start of the current sentence (nearest
    '.', '!', '?', or newline).  Lower-cased for case-insensitive matching.
    """
    window_start = max(0, match_start - _ASSERTION_LOOKBACK_CHARS)
    snippet = text[window_start:match_start]
    # Clip at the most recent sentence terminator so we don't leak context
...
def _classify_assertion(preceding: str) -> tuple[bool, bool, str | None]:
    """
    Inspect the lower-cased preceding-context window and decide whether the
    ICD mention is negated or uncertain.

    Returns (negated, uncertain, matched_trigger).  Negation takes priority
    over uncertainty if both are present.
    """
    for trig in NEGATION_TRIGGERS:
        # Use word-ish boundaries so "history of" doesn't spuriously match
    ...
    for trig in UNCERTAIN_TRIGGERS:
        if trig == "vs" or trig == "versus":
            # Require surrounding whitespace so we don't trigger on
            # "vstachy..." or random substrings.
            if f" {trig} " in f" {preceding} ":
                return False, True, trig
            continue
        if trig in preceding:
            return False, True, trig
...
def extract_icd_codes(text: str) -> list[dict]:
    """
    Find ALL ICD-10-CM codes in *text* using regex, then filter out
    mentions that appear in a negated or uncertain context.

    Pattern: [A-TV-Z]\\d{2}(\\.[A-Z0-9]{1,4})? with word boundary enforcement.
    Matches inside parentheses or square brackets are captured correctly
    because '(' and '[' are non-word characters.

    Assertion handling (rule-based — see module docstring for limitations):
    for each candidate match we look at the ~60 chars BEFORE the code in
    the same sentence.  If a NEGATION_TRIGGERS phrase is present, the
    mention is marked ``negated=True`` and EXCLUDED from the returned
    positive list.  If an UNCERTAIN_TRIGGERS phrase is present, the
    mention is marked ``uncertain=True`` and also excluded.  Both sets
    are logged (and exposed on the returned dicts' metadata-in-context)
    so downstream consumers / reviewers can audit what was filtered.

    Returns a list of dicts (positive findings only) where each dict has:
        ...
        negated     — always False in the returned list
        uncertain   — always False in the returned list
        assertion_trigger — None for positives

    Negated/uncertain entries are *not* returned here to preserve the
    existing caller contract (downstream code in this module treats
    every entry as a positive finding).  They are emitted via logger
    at DEBUG level for transparency.
    """
    if not text:
        return []

    positive: list[dict] = []
    negated_codes: list[dict] = []
    uncertain_codes: list[dict] = []
    ...
    for match in _ICD10_PATTERN.finditer(text):
        ...
        preceding = _preceding_window(text, start)
        negated, uncertain, trigger = _classify_assertion(preceding)
        ...
        if negated:
            negated_codes.append(entry)
            continue
        if uncertain:
            uncertain_codes.append(entry)
            continue
        positive.append(entry)

    if negated_codes:
        logger.info(
            "[Stage1] Filtered %d negated ICD mention(s): %s",
            len(negated_codes),
            [(e["code"], e["assertion_trigger"], e["line_number"]) for e in negated_codes],
        )
    if uncertain_codes:
        logger.info(
            "[Stage1] Filtered %d uncertain ICD mention(s): %s",
            len(uncertain_codes),
            [(e["code"], e["assertion_trigger"], e["line_number"]) for e in uncertain_codes],
        )
