...
def get_meat_evidence(patient_id: int, year: int = 2026) -> list[dict[str, Any]]:
...
def calculate_meat_completeness(patient_id: int, year: int = 2026) -> dict[str, Any]:
    ...
    * encounter_date is read from gemini_result["_meta"]["encounter_date"] if
    ...
    with raf_cursor() as cur:
        ...
        hcc_map: dict[int, int] = {
            int(r["hcc_code"]): int(r["id"]) for r in cur.fetchall()
        }
...
def update_hcc_meat_status(patient_id: int, year: int = 2026) -> None:
