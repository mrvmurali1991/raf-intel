...
class TestHealthContract:
    ...
    def test_health_returns_json_with_status_field(self, client) -> None:
        resp = client.get("/health")
        body = resp.json()
...
class TestMeContract:
    ...
    def test_me_returns_200_for_authenticated_user(self, client) -> None:
        user = dict(MOCK_ADMIN_USER)
        with _auth_as(user):
            resp = client.get("/api/auth/me", headers=_headers(user))
        assert resp.status_code == 200

    def test_me_response_contains_required_fields(self, client) -> None:
        user = dict(MOCK_ADMIN_USER)
        with _auth_as(user):
            resp = client.get("/api/auth/me", headers=_headers(user))
        body = resp.json()
    ...
    def test_me_response_does_not_leak_password_hash(self, client) -> None:
        user = dict(MOCK_ADMIN_USER)
        with _auth_as(user):
            resp = client.get("/api/auth/me", headers=_headers(user))
        body = resp.json()
    ...
    def test_me_returns_correct_email(self, client) -> None:
        user = dict(MOCK_ADMIN_USER)
        with _auth_as(user):
            resp = client.get("/api/auth/me", headers=_headers(user))
        assert resp.json().get("email") == user["email"]

    def test_me_returns_correct_tenant_id(self, client) -> None:
        user = dict(MOCK_ADMIN_USER)
        with _auth_as(user):
            resp = client.get("/api/auth/me", headers=_headers(user))
        assert resp.json().get("tenant_id") == user["tenant_id"]

    def test_me_returns_401_without_token(self, client) -> None:
        resp = client.get("/api/auth/me")
        assert resp.status_code == 401

    def test_me_mfa_secret_not_in_response(self, client) -> None:
...
class TestLoginContract:
    ...
    def test_login_with_missing_email_returns_422(self, client) -> None:
        resp = client.post("/api/auth/login", json={"password": "Admin@123"})
        assert resp.status_code == 422

    def test_login_with_missing_password_returns_422(self, client) -> None:
        resp = client.post("/api/auth/login", json={"email": "admin@raf.health"})
        assert resp.status_code == 422

    def test_login_with_invalid_credentials_returns_401(self, client) -> None:
    ...
    def test_login_success_returns_access_token(self, client) -> None:
        user = dict(MOCK_ADMIN_USER)
        with (
    ...
    def test_login_response_has_no_password_hash(self, client) -> None:
...
class TestRefreshContract:
    def test_refresh_without_token_returns_401_or_422(self, client) -> None:
        resp = client.post("/api/auth/refresh")
        assert resp.status_code in (401, 422)

    def test_refresh_with_expired_token_returns_401(self, client) -> None:
        user = dict(MOCK_ADMIN_USER)
...
class TestPatientListContract:
    ...
    def test_patient_list_returns_200(self, client) -> None:
        user = dict(MOCK_ADMIN_USER)
        with (
            _auth_as(user),
            patch(
    ...
    def test_patient_list_has_required_envelope_fields(self, client) -> None:
        user = dict(MOCK_ADMIN_USER)
        with (
            _auth_as(user),
            patch(
    ...
    def test_patient_list_requires_auth(self, client) -> None:
        resp = client.get("/api/patients")
        assert resp.status_code == 401

    def test_patient_list_patients_is_list(self, client) -> None:
        user = dict(MOCK_ADMIN_USER)
        with (
            _auth_as(user),
            patch(
...
class TestPatientDetailContract:
    ...
    def test_patient_detail_404_for_nonexistent_pid(self, client) -> None:
        user = dict(MOCK_ADMIN_USER)
        with (
            _auth_as(user),
    ...
    def test_patient_detail_returns_patient_data_fields(self, client) -> None:
        user = dict(MOCK_ADMIN_USER)
        mock_patient = {
            "pid": 1,
            "first_name": "Alice",
    ...
    def test_patient_detail_requires_auth(self, client) -> None:
...
class TestRAFContract:
    ...
    def test_population_summary_requires_auth(self, client) -> None:
        resp = client.get("/api/raf/population-summary")
        assert resp.status_code == 401

    def test_raf_list_models_returns_list(self, client) -> None:
    ...
    def test_raf_score_history_requires_auth(self, client) -> None:
...
class TestPipelineContract:
    ...
    def test_pipeline_status_requires_auth(self, client) -> None:
        resp = client.get("/api/emr/pipeline/status")
        assert resp.status_code in (401, 404)

    def test_pipeline_trigger_requires_auth(self, client) -> None:
        resp = client.post("/api/emr/sync")
        assert resp.status_code == 401

    def test_emr_connections_list_requires_auth(self, client) -> None:
        resp = client.get("/api/emr/connections")
        assert resp.status_code == 401

    def test_emr_connections_list_returns_list(self, client) -> None:
        user = dict(MOCK_ADMIN_USER)
        with (
            _auth_as(user),
            patch(
...
class TestAdminContract:
    ...
    def test_admin_users_list_forbidden_for_viewer(self, client) -> None:
        user = dict(MOCK_VIEWER_USER)
        with _auth_as(user):
            resp = client.get("/api/auth/users", headers=_headers(user))
        assert resp.status_code == 403

    def test_admin_users_list_returns_list_for_admin(self, client) -> None:
        user = dict(MOCK_ADMIN_USER)
        with (
            _auth_as(user),
            _noop_raf_cursor(),
    ...
    def test_audit_log_requires_auth(self, client) -> None:
