...
def test_perfect_predictions_yield_f1_one(tmp_path) -> None:
    charts = [
...
def test_all_misses_yield_zero(tmp_path) -> None:
...
def test_partial_match_metrics(tmp_path) -> None:
...
def test_macro_vs_micro_differ_with_class_imbalance(tmp_path) -> None:
...
def test_llm_only_uses_only_mock(tmp_path) -> None:
...
def test_kg_first_uses_kg_when_llm_silent(tmp_path) -> None:
...
def test_hybrid_unions_both(tmp_path) -> None:
...
def test_load_fixtures_happy_path() -> None:
...
def test_load_fixtures_rejects_missing_id(tmp_path) -> None:
    fp = _write_fixture(tmp_path, [{"gold_hccs": []}])
    with pytest.raises(ValueError, match="missing required field 'id'"):
        kg_benchmark.load_fixtures(fp)


def test_load_fixtures_rejects_missing_gold(tmp_path) -> None:
...
def test_load_fixtures_missing_file() -> None:
    with pytest.raises(FileNotFoundError):
        kg_benchmark.load_fixtures("/tmp/does-not-exist-xyz.json")


def test_load_fixtures_accepts_bare_list(tmp_path) -> None:
...
def test_kg_medication_signal_metformin() -> None:
...
def test_kg_lab_signal_egfr_stage_4() -> None:
    res = kg_lookup_service.patient_full_inference({
...
def test_kg_problem_list_dm_neuropathy_chain() -> None:
    res = kg_lookup_service.patient_full_inference({
        "id": "x",
...
def test_kg_recapture_gap_emitted() -> None:
    res = kg_lookup_service.patient_full_inference({
        "id": "x",
        "prior_year_hccs": ["280"],
        "current_year_hccs": [],
...
def test_kg_drops_predictions_without_v28_hcc() -> None:
...
def test_custom_gemini_caller_is_used(tmp_path) -> None:
    charts = [{"id": "a", "gold_hccs": ["38"], "llm_mock": []}]
    fp = _write_fixture(tmp_path, charts)
...
def test_extended_fixture_runs_all_modes_without_error() -> None:
...
def test_kg_first_beats_llm_only_or_matches_on_recall() -> None:
...
def test_render_markdown_report_contains_required_sections(tmp_path) -> None:
    fp = FIXTURE_DIR / "extended_charts.json"
